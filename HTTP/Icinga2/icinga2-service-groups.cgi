...
if ($Add_Group) {
	...
	print "Location: /Icinga2/icinga2-service-groups.cgi\n\n";
	exit(0);
}
elsif ($Edit_Group) {
	require $Header;
...
<a href="/Icinga2/icinga2-service-groups.cgi">
...
<form action='/Icinga2/icinga2-service-groups.cgi' method='post' >
...
sub add_group {
	...
	WHERE `servicegroup_name` = '$Group_Add'");
	...
	if ($Group_Rows) {
		...
		my $Group_Insert = $DB_Connection->prepare("INSERT INTO `icinga2_servicegroup` (
			`id`,
			`servicegroup_name`,
			`alias`,
			`active`,
...
sub html_edit_group {
	...
	my $Select_Group = $DB_Connection->prepare("SELECT `servicegroup_name`, `alias`, `active`
	FROM `icinga2_servicegroup`
	WHERE `id` = '$Edit_Group'");
...
sub edit_group {
	...
	my $Group_Insert_Check = $DB_Connection->prepare("SELECT `id`, `alias`
	FROM `icinga2_servicegroup`
	WHERE `servicegroup_name` = '$Group_Edit'
	...
	if ($Group_Rows) {
		...
		my $Group_Update = $DB_Connection->prepare("UPDATE `icinga2_servicegroup` SET
			`servicegroup_name` = ?,
			`alias` = ?,
			`active` = ?,
			`last_modified` = NOW(),
...
sub html_delete_group {
	...
	my $Select_Group = $DB_Connection->prepare("SELECT `servicegroup_name`, `alias`
	FROM `icinga2_servicegroup`
...
sub delete_group {
	...
	$DB_Connection->do("DELETE from `icinga2_servicegroup`
...
sub html_display_config {
	...
	my $Members;
	my $Select_Group = $DB_Connection->prepare("SELECT `servicegroup_name`, `alias`, `active`, `last_modified`, `modified_by`
	FROM `icinga2_servicegroup`
	WHERE `id` = ?");
	...
	while ( my @DB_Group = $Select_Group->fetchrow_array() )
	{
		...
		if (!$Active_Extract) {
			...
			This service group is not active, so this config will not be written. 
			Make this service group active to use it in Icinga.</span>";
		}
		else {
			$Active_Extract="";
		}
...
<p>This config is automatically applied regularly. The config below illustrates how this service group's config will be written.</p>
<p>$Active_Extract</p>
<div style="text-align: left;">
<code>
<table align = "center">
	<tr>
		<td colspan='3'>## Service Group ID: $Display_Config</td>
	</tr>
	<tr>
		<td colspan='3'>## Modified $Last_Modified_Extract by $Modified_By_Extract</td>
	</tr>
	<tr>
		<td colspan='3'>define servicegroup {</td>
	</tr>
	<tr>
		<td style='padding-left: 2em;'>servicegroup_name</td>
...
sub html_output {
	...
	my $Select_Groups_Count = $DB_Connection->prepare("SELECT `id` FROM `icinga2_servicegroup`");
		$Select_Groups_Count->execute( );
		my $Total_Rows = $Select_Groups_Count->rows();

	my $Select_Groups = $DB_Connection->prepare("SELECT `id`, `servicegroup_name`, `alias`, `active`, `last_modified`, `modified_by`
	FROM `icinga2_servicegroup`
	WHERE (`id` LIKE '%$Filter%'
	OR `servicegroup_name` LIKE '%$Filter%'
	...
	while ( my @DB_Group = $Select_Groups->fetchrow_array() )
	{
		...
		$Table->addRow(
			"<a href='/Icinga2/icinga2-service-groups.cgi?Edit_Group=$ID_Extract'>$ID_Extract_Display</a>",
			"<a href='/Icinga2/icinga2-service-groups.cgi?Edit_Group=$ID_Extract'>$Name_Extract</a>",
			$Alias_Extract,
			$Active_Extract,
			$Last_Modified_Extract,
			$Modified_By_Extract,
			"<a href='/Icinga2/icinga2-service-groups.cgi?Display_Config=$ID_Extract'><img src=\"/Resources/Images/view-notes.png\" alt=\"View Config for $Name_Extract\" ></a>",
			"<a href='/Icinga2/icinga2-service-groups.cgi?Edit_Group=$ID_Extract'><img src=\"/Resources/Images/edit.png\" alt=\"Edit $Name_Extract\" ></a>",
			"<a href='/Icinga2/icinga2-service-groups.cgi?Delete_Group=$ID_Extract'><img src=\"/Resources/Images/delete.png\" alt=\"Delete $Name_Extract\" ></a>"
		);
...
<p style="font-size:14px; font-weight:bold;">Icinga Service Groups | Groups Displayed: $Rows of $Total_Rows</p>
