	...
	while ( my @DB_Output = $Discover_Allocatable_Blocks->fetchrow_array() ) {
		...
		LOOP: while ($Counter != $Rows) {
			...
			while ( my @Block_DB_Output = $IP_Block_Check_Cycle->fetchrow_array() )
			{
				...
				my $IP_Assignment = new Net::IP::XS($IP_Block_For_Assignment) or die(&assignment_error(Net::IP::XS::Error, $IP_Block_For_Assignment));
					my $Proposed_IP = $IP_Assignment->ip();
				my $IP_Assignment_Check = new Net::IP::XS($Network_Block) or die(&assignment_error(Net::IP::XS::Error, $Network_Block));
				...
				if (not defined($Overlap_Check))
				{
					...
					$Session->param('Message_Red', $Message_Red);
					$Session->flush();
					print "Location: /IP/ipv4-assignments.cgi?Reset=1\n\n";
					exit(0);
				}
		...
		my $Final_IP_Assignment = new Net::IP::XS ($Final_Block) or die(&assignment_error(Net::IP::XS::Error, $Final_Block));
			my $IP_Block_Limit_Final = $Final_IP_Assignment->last_ip();
				my $IP_Assignment_Limit_Final = new Net::IP::XS ($IP_Block_Limit_Final) or die(&assignment_error(Net::IP::XS::Error, $IP_Block_Limit_Final));
...
my $IP_Assignment = new Net::IP::XS ($IP_Block) or die(&assignment_error(Net::IP::XS::Error, $IP_Block));

	$IP_Block = $IP_Assignment->intip();
...
<a href="/IP/ipv4-assignments.cgi">
<div id="blockclosebutton">
</div>
</a>

<h3 align="center">IPv4 Assignment</h3>
...
sub add_block {
	...
	my $Existing_Block_Check = $DB_Connection->prepare("SELECT `id`
		FROM `ipv4_assignments`
		WHERE `ip_block` = ?");
		$Existing_Block_Check->execute($Final_Assignment);
	...
	if ($Existing_Blocks > 0)  {
		...
		my $Message_Red="Block: $Final_Assignment already exists as ID: $Existing_ID";
		$Session->param('Message_Red', $Message_Red);
		$Session->flush();
		print "Location: /IP/ipv4-assignments.cgi\n\n";
		exit(0);
	...
	$Block_Insert->execute($Final_Assignment, $Final_Parent, $User_Name);
	...
	foreach my $Host (@Host) {
		$Host_Counter++;
		my $Host_Insert = $DB_Connection->prepare("INSERT INTO `lnk_hosts_to_ipv4_assignments` (
			`host`,
			`ip`
		)
		VALUES (
...
<h3 align="center">Edit IPv4 Assignment ID $Edit_Block</h3>
<h4 align="center">Existing Assignment Associations</h4>

<form action='/IP/ipv4-assignments.cgi' method='post' >
...
sub edit_block {
	...
	$Audit_Log_Submission->execute("IP", "Modify", "$User_Name added $Host_Counter new hosts to IPv4 Assignment ID $Block_Edit.", $User_Name);
...
sub delete_block {
	...
	while (( my $Block ) = $Select_Blocks->fetchrow_array() )
	{
		my $DB_Connection = DB_Connection();
		my $Audit_Log_Submission = Audit_Log_Submission();
		$Audit_Log_Submission->execute("IP", "Delete", "$User_Name deleted $Block (IPv4 Assignment ID $Delete_Block_Confirm).", $User_Name);
	}
	# / Audit Log
...
$Table->addRow( "ID", 
"IP/Block<a href='/IP/ipv4-assignments.cgi?Order_By=$Order_By_Block&Filter=$Filter'>$Block_IP_Arrow</a>", 
...
while ( my @Select_Blocks = $Select_Blocks->fetchrow_array() ) {
	...
	$Hosts =~ s/,&nbsp;$//;
	$Table->addRow( $ID, 
	"<a href='/IP/ipv4-assignments.cgi?Query_Block=$Block_Extract&Query_Parent=$Parent_Block_Extract'>$Block</a>",
	$Hosts,
...
print <<ENDHTML;
		...
		<td align="right">
			<a href="ipv4-assignments.cgi?Manual_Override=1"><span style="color: #FF8A00;">Switch to Manual Assignment Mode</span></a>
		</td>
	</tr>
</table>

<p style="font-size:14px; font-weight:bold;">IPv4 Assignments | Blocks Displayed: $Rows of $Total_Rows</p>
...
<h2 style='text-align:center;'>IPv4 Manual Assignment</h2>
...
<h4 style='text-align:center; color: #00FF00;'><a href='ipv4-assignments.cgi'>Click here to switch back to Automatic Assignment Mode</a></h4>
