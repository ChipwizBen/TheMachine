...
if ($Add_Group) {
	...
	print "Location: nagios-host-groups.cgi\n\n";
	exit(0);
}
elsif ($Edit_Group) {
	require "header.cgi";
...
sub add_group {
	...
	if ($Group_Rows) {
		...
		my $Group_Insert = $DB_Icinga->prepare("INSERT INTO `nagios_hostgroup` (
			`id`,
			`hostgroup_name`,
			`alias`,
			`active`,
...
sub html_edit_group {
	...
	my $Select_Group = $DB_Icinga->prepare("SELECT `hostgroup_name`, `alias`, `active`
	FROM `nagios_hostgroup`
	WHERE `id` = '$Edit_Group'");
...
sub edit_group {
	...
	if ($Group_Rows) {
		...
		my $Group_Update = $DB_Icinga->prepare("UPDATE `nagios_hostgroup` SET
			`hostgroup_name` = ?,
			`alias` = ?,
			`active` = ?,
			`last_modified` = NOW(),
...
sub html_delete_group {
	...
	my $Select_Group = $DB_Icinga->prepare("SELECT `hostgroup_name`, `alias`
	FROM `nagios_hostgroup`
...
sub delete_group {
	...
	$DB_Icinga->do("DELETE from `nagios_hostgroup`
...
sub html_display_config {
	...
	my $Members;
	my $Select_Group = $DB_Icinga->prepare("SELECT `hostgroup_name`, `alias`, `active`, `last_modified`, `modified_by`
	FROM `nagios_hostgroup`
	WHERE `id` = ?");
	...
	while ( my @DB_Group = $Select_Group->fetchrow_array() )
	{
		...
		while ( my @DB_Members = $Select_Members->fetchrow_array() )
		{
			...
			my $Select_Member_Names = $DB_Icinga->prepare("SELECT `host_name`
			FROM `nagios_host`
			WHERE `id` = ?");
...
sub html_output {
	...
	my $Select_Groups_Count = $DB_Icinga->prepare("SELECT `id` FROM `nagios_hostgroup`");
		$Select_Groups_Count->execute( );
		my $Total_Rows = $Select_Groups_Count->rows();

	my $Select_Groups = $DB_Icinga->prepare("SELECT `id`, `hostgroup_name`, `alias`, `active`, `last_modified`, `modified_by`
	FROM `nagios_hostgroup`
	...
	while ( my @DB_Group = $Select_Groups->fetchrow_array() )
	{
		...
		$Table->addRow(
			...
			$Modified_By_Extract,
			"<a href='Icinga/icinga-host-groups.cgi?Display_Config=$ID_Extract'><img src=\"resorcs/imgs/view-notes.png\" alt=\"View Config for $Name_Extract\" ></a>",
			"<a href='Icinga/icinga-host-groups.cgi?Edit_Group=$ID_Extract'><img src=\"resorcs/imgs/edit.png\" alt=\"Edit $Name_Extract\" ></a>",
			"<a href='Icinga/icinga-host-groups.cgi?Delete_Group=$ID_Extract'><img src=\"resorcs/imgs/delete.png\" alt=\"Delete $Name_Extract\" ></a>"
		);
