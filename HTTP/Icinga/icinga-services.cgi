...
sub add_service {
	...
	my $Service_Insert = $DB_Icinga->prepare("INSERT INTO `nagios_service` (
	...
	if ($Template_Add) {
		...
		my $Template_Insert = $DB_Icinga->prepare("INSERT INTO `nagios_lnkServiceToServicetemplate` (
		`idMaster`, `idSlave`
		)
		VALUES (
		?, ?
	...
	if ($Contact_Add) {
		...
		my $Contact_Insert = $DB_Icinga->prepare("INSERT INTO `nagios_lnkServiceToContact` (
		`idMaster`, `idSlave`
		)
		VALUES (
		?, ?
	...
	if ($Contact_Group_Add) {
		...
		my $Contact_Group_Insert = $DB_Icinga->prepare("INSERT INTO `nagios_lnkServiceToContactgroup` (
		`idMaster`, `idSlave`
		)
		VALUES (
		?, ?
	...
	if ($Service_Group_Add) {
		...
		my $Contact_Group_Insert = $DB_Icinga->prepare("INSERT INTO `nagios_lnkServiceToServicegroup` (
		`idMaster`, `idSlave`
		)
		VALUES (
		?, ?
...
sub html_edit_service {
	...
	my $Select_Service = $DB_Icinga->prepare("SELECT `config_name`, `service_description`, `active`
	FROM `nagios_service`
...
sub html_delete_service {
	...
	my $Select_Service = $DB_Icinga->prepare("SELECT `id`, `service_description`
	FROM `nagios_service`
...
sub delete_service {
	...
	my $Delete = $DB_Icinga->prepare("DELETE from `nagios_service`
				WHERE `id` = ?");
	$Delete->execute($Service_Delete_Post);

	$Delete = $DB_Icinga->prepare("DELETE from `nagios_lnkServiceToServicetemplate`
				WHERE `idMaster` = ?");
	$Delete->execute($Service_Delete_Post);

	$Delete = $DB_Icinga->prepare("DELETE from `nagios_lnkServiceToContact`
				WHERE `idMaster` = ?");
	$Delete->execute($Service_Delete_Post);

	$Delete = $DB_Icinga->prepare("DELETE from `nagios_lnkServiceToContactgroup`
				WHERE `idMaster` = ?");
	$Delete->execute($Service_Delete_Post);

	$Delete = $DB_Icinga->prepare("DELETE from `nagios_lnkServiceToServicegroup`
				WHERE `idMaster` = ?");
	$Delete->execute($Service_Delete_Post);
...
sub html_display_config {
	...
	my $Select_Service = $DB_Icinga->prepare("SELECT `service_description`, `active_checks_enabled`, `check_freshness`, 
	...
	while ( my @DB_Service = $Select_Service->fetchrow_array() )
	{
		...
		while ( my @DB_Service_Template_ID = $Select_Service_Template_ID->fetchrow_array() )
		{
			...
			my $Select_Service_Template = $DB_Icinga->prepare("SELECT `template_name`, `active_checks_enabled`, `check_freshness`, 
			...
			## Template Contact Link Collection
				...
				my $Select_Contact_Link = $DB_Icinga->prepare("SELECT `idSlave`
				...
				while ( my @DB_Host_Link = $Select_Contact_Link->fetchrow_array() )
				{
					...
					my $Select_Contact = $DB_Icinga->prepare("SELECT `contact_name`
					FROM `nagios_contact`
					WHERE `id` = '$Host_ID'
					AND `active` = '1'");
			...
			## Template Contact Group Link Collection
				...
				my $Select_Contact_Group_Link = $DB_Icinga->prepare("SELECT `idSlave`
				...
				while ( my @DB_Host_Link = $Select_Contact_Group_Link->fetchrow_array() )
				{
					...
					my $Select_Contact = $DB_Icinga->prepare("SELECT `contactgroup_name`
					FROM `nagios_contactgroup`
					WHERE `id` = '$Host_ID'
					AND `active` = '1'");
					$Select_Contact->execute();
...
sub html_linked_hosts {
	...
	my $Select_Service_Name = $DB_Icinga->prepare("SELECT `service_description`
	FROM `nagios_service`
	WHERE `id` = ?");
	...
	$Select_Host_Name_Link->execute();
		...
		while ( my @DB_Host_Link = $Select_Host_Name_Link->fetchrow_array() )
		{
			...
			my $Select_Host_Name = $DB_Icinga->prepare("SELECT `host_name`, `active`
			FROM `nagios_host`
			WHERE `id` = '$Host_ID'");
			$Select_Host_Name->execute();
...
sub html_output {
	...
	my $Select_Services_Count = $DB_Icinga->prepare("SELECT `id` FROM `nagios_service`");
	...
	while ( my @DB_Service = $Select_Services->fetchrow_array() )
	{
		...
		my $Select_Service_Template_Link = $DB_Icinga->prepare("SELECT `idSlave`
		FROM `nagios_lnkServiceToServicetemplate`
